...
class LoggerManager(QObject):
    ...
    def __init__(self):
        ...
        if self._logger is None:
            self._setup_logger()
            # 自动设置文件日志
            self._auto_setup_file_logging()
        self._main_thread_id = threading.get_ident()

    def _auto_setup_file_logging(self):
        """在初始化时自动设置文件日志"""
        try:
            today = datetime.now().strftime("%Y%m%d")
            log_file_path = os.path.join(os.getcwd(), f"pinzimu_{today}_debug.log")

            # 检查是否已经有文件处理器，避免重复添加
            has_file_handler = any(isinstance(handler, (logging.FileHandler, RotatingFileHandler))
            ...
            print(f"✅ 日志管理器已初始化，日志文件: {log_file_path}")

        except Exception as e:
            print(f"❌ 自动设置文件日志失败: {e}")
    ...
    def _setup_logger(self):
        """设置日志器"""
        self._logger = logging.getLogger('demucs_gui')
        ...
        console_handler.setLevel(logging.INFO)  # 控制台日志级别设为INFO
    ...
    def setup_file_logging(self, log_file_path: str = None, enable_debug: bool = True):
        ...
        try:
            ...
            # 写入会话开始标记
            with open(log_file_path, 'a', encoding='utf-8') as f:
                f.write(f"\n=== 调试模式切换 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ===\n")
    ...
    def log(self, level: str, message: str, module_name: str = None, show_in_ui: bool = False):
        ...
        logger = self._logger
        if module_name:
            # 创建子日志器并设置不传播到父日志器，避免重复记录
            logger = logging.getLogger(f'demucs_gui.{module_name}')
